...
def _heading_orange_support(headings, screenshot_path, window_rect):
    """Return per-heading visual support from the actual captured pixels.

    Chromium can leave stale accessibility nodes from prior mini-program pages
    marked as visible.  Rating headings have a stable orange/red foreground,
    so we corroborate UIA geometry against the screenshot before calling a
    page RATING_FORM.  This is not OCR: it simply checks that the pixels inside
    each exact UIA heading rectangle contain the expected heading colour.
    """
    result = {}
    ...
    for english, node in headings.items():
        ...
        for red, green, blue in crop.getdata():
            # Broad enough for antialiasing / display colour variation, while
            # excluding black description text and purple/grey roster badges.
        ...
        result[english] = {
            ...
            "supported": orange >= 40 and (orange / total if total else 0.0) >= 0.045,
        }
    return result
